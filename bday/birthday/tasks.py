...
def send_scheduled_wishes_task():
    ...
    for wish in pending_wishes:
        ...
        template_type = getattr(template, 'type', 'Email')
        ...
        try:
            ...
            if template_type == 'SMS':
                if not patient.phone:
                    wish.status = 'Failed'
                    wish.error_message = 'Patient has no phone number'
                    wish.save()
                    failed_count += 1
                    print(f'  ✗ {patient}: No phone number for SMS')
                    continue
                ...
                if success:
                    ...
                    sent_count += 1
                    print(f'  ✓ {patient}: SMS sent successfully')
                else:
                    wish.status = 'Failed'
                    wish.error_message = result
                    wish.save()
                    failed_count += 1
                    print(f'  ✗ {patient}: SMS failed - {result}')
